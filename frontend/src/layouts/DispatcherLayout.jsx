...
function getDispatcherIdentity() {
  try {
...
export function DispatcherLayout() {
  ...
  const identity = getDispatcherIdentity();
  const dispatcherName = identity.name || identity.full_name || identity.username || identity.email || identity.sub || '';
  const dispatcherRole = identity.role?.replaceAll('_', ' ') || '';
  ...
  return (
    <div className={`dispatcher-app-shell dispatcher-layout${isDriversPage ? ' drivers-figma-view' : ''}${isVehiclesPage ? ' vehicles-figma-view' : ''}${isShiftsPage ? ' shifts-figma-view' : ''}${isDispatchQueuePage ? ' dispatch-figma-view' : ''}${isRoutesPage ? ' routes-figma-view' : ''}`}>
      <aside className="dispatcher-sidebar">
        ...
        <div className="dispatcher-account-footer">
          <div className="dispatcher-account-row">
            <span className="dispatcher-user-avatar" aria-hidden="true">{getInitials(String(dispatcherName))}</span>
            <span className="dispatcher-user-copy"><strong>{dispatcherName}</strong><small>{dispatcherRole}</small></span>
          </div>
